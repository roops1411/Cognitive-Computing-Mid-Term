...
conf['samples_train_per_epoch'] = 25
# Number of random samples to use during validation per epoch
conf['samples_valid_per_epoch'] = 20
# Some variables to control CNN structure
conf['level_1_filters'] = 3
conf['level_2_filters'] = 4
conf['dense_layer_size'] = 32
conf['dropout_value'] = 0.25
...
def get_custom_CNN():
    model = Sequential()
    model.add(ZeroPadding2D((1,1,1, 1), input_shape=(1, conf['image_shape'][0], conf['image_shape'][1]), dim_ordering='th'))
...
def create_single_model():
    ...
    train_csv_table = pd.read_csv('/home/ec2-user/sample_images/stage1_labels.csv')
    ...
    if conf['use_sample_only'] == 1:
        get_dir = '00cba091fa4ad62cc3200a657aeb957e'

    train_files = []
    for p in train_patients:
        train_files += glob.glob("/home/ec2-user/sample_images/{}/{}/*.dcm".format(get_dir, p))
    print('Number of train files: {}'.format(len(train_files)))

    valid_files = []
    for p in valid_patients:
        valid_files += glob.glob("/home/ec2-user/sample_images/{}/{}/*.dcm".format(get_dir, p))
...
def create_submission(model):
    sample_subm = pd.read_csv("/home/ec2-user/sample_images/stage1_sample_submission.csv")
    ids = sample_subm['id'].values
 
    for id in ids:
        print('Predict for patient {}'.format(id))
        files = glob.glob("/home/ec2-user/sample_images/stage1/{}/*.dcm".format(id))
        image_list = []
        for f in files:
            image = load_and_normalize_dicom(f, conf['image_shape'][0], conf['image_shape'][1])
            image_list.append([image])
    ...
    sample_subm.to_csv("subm3.csv", index=False)
